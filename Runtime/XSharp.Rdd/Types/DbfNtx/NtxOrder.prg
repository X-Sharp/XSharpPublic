...
PRIVATE METHOD _locateKey(abKey AS BYTE[] , uiLen AS LONG , searchMode AS NtxSearchMode ) AS DWORD
SELF:_TopStack := 0u
IF (uiLen > SELF:_keySize)
uiLen := SELF:_keySize
ELSE
...
ENDIF
ENDIF
RETURN SELF:_locate(abKey, uiLen, searchMode, (INT)SELF:_firstPageOffset)


PRIVATE METHOD _locate(abKey AS BYTE[] , uiLen AS LONG , searchMode AS NtxSearchMode , lPage AS LONG ) AS DWORD
LOCAL count AS LONG
LOCAL ntxPage AS NtxPage
LOCAL nodeCount AS LONG
LOCAL node AS NtxItem
...
count := loCount
node:Fill(count, ntxPage)
IF ((searchMode == NtxSearchMode.Left) .AND. (SELF:_oRdd:StringCompare(node:KeyBytes, abKey, uiLen) == 0))
searchMode := NtxSearchMode.LeftFound
ENDIF
...
node:Fill(count, ntxPage)
IF (node:PageNo != 0)
RETURN SELF:_locate(abKey, uiLen, searchMode, node:PageNo)
ENDIF
IF (count < nodeCount)
BEGIN SWITCH searchMode
CASE NtxSearchMode.LeftFound
