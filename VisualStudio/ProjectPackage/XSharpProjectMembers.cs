...
namespace XSharp.Project
{
    ...
    internal static class XSharpProjectMembers
   {
      ...
      internal static void AddNonMemberItems( XSharpProjectNode project )
      {
      ...
      /// <param name="project">The project to modify.</param>
      [SuppressMessage( "Microsoft.Naming", "CA1702:CompoundWordsShouldBeCasedCorrectly", MessageId = "NonMember" )]
      internal static void RemoveNonMemberItems( XSharpProjectNode project )
      {
      ...
      private static void ExcludeProjectBuildItems( XSharpProjectNode project, IList<string> fileList, IList<string> folderList )
      {
         ...
         foreach (MSBuild.ProjectItem buildItem in projectItems)
        {
                ...
                String.Equals(buildItem.ItemType, ProjectFileConstants.Folder, StringComparison.OrdinalIgnoreCase))
            {
               ...
               if ( folderMap.ContainsKey( relativePath ) )
               {
                ...
                fileMap.Count > 0 &&
                XSharpProjectMembers.IsXSharpFileItem( buildItem ) )
            {
               string relativePath = buildItem.EvaluatedInclude;
               if ( Path.IsPathRooted( relativePath ) ) // if not the relative path, make it relative
               {
      ...
      [SuppressMessage( "Microsoft.Naming", "CA1702:CompoundWordsShouldBeCasedCorrectly", MessageId = "NonMember" )]
      private static void AddNonMemberFolderItems( XSharpProjectNode project, IList<string> folderList )
      {
         if ( folderList == null )
         {
            throw new ArgumentNullException( "folderList" );
      ...
      [SuppressMessage( "Microsoft.Naming", "CA1702:CompoundWordsShouldBeCasedCorrectly", MessageId = "NonMember" )]
      private static void AddNonMemberFileItems( XSharpProjectNode project, IList<string> fileList )
      {
         if ( fileList == null )
         {
      ...
      private static bool IsXSharpFileItem(MSBuild.ProjectItem buildItem)
      {
         ...
         return String.Equals( name, ProjectFileConstants.Compile, StringComparison.OrdinalIgnoreCase) ||
                ...
                String.Equals( name, "NativeResource", StringComparison.OrdinalIgnoreCase) ||
                String.Equals( name, "VOBinary", StringComparison.OrdinalIgnoreCase)
