...
SEALED CLASS EntityParser //IMPLEMENTS ILanguageService
	...
	VIRTUAL METHOD ParseModule(oModule AS Module) AS LOGIC
		...
		TRY
			SELF:lParsing := TRUE
			oGlobalScope := TypeInfo{"(Global Scope)" , ElementType.Class , AccessType.Public , 0 , 0 , 0 ,0}
			...
			FOR n := 0 UPTO aEntities:Count - 1
				...
				IF IsType(oInfo:eType)
					oType := TypeInfo{oInfo:cNameSpace + oInfo:cName , EntityToElementType(oInfo:eType) , AccessLevelToAccessType(oInfo:eAccessLevel) , oInfo:nLine , 0 , 0 , 0}
				...
				ELSE
					...
					OTHERWISE
						cName := oInfo:cName
					END CASE
					oMember := @@Member{cName , EntityToElementType(oInfo:eType) , AccessLevelToAccessType(oInfo:eAccessLevel) , oInfo:nLine , 0 , 0 , 0}
	...
	PROTECTED STATIC METHOD IsType(eType AS EntityType) AS LOGIC
