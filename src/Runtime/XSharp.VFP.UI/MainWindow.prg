...
BEGIN NAMESPACE XSharp.VFP.UI
	...
	CLASS MainWindow INHERIT Form
		...
		// ── Singleton ─────────────────────────────────────────────────────────
		/// <summary>The singleton <see cref="MainWindow"/> instance. Set in the constructor so <c>Form.prg</c> can reach <c>_Screen</c> without depending on the generated <c>_Screen</c> GLOBAL in another assembly.</summary>
		STATIC PROPERTY Current AS MainWindow AUTO

		PRIVATE _Forms AS List<Form>

		// ── ActiveMenu ────────────────────────────────────────────────────────
		/// <summary>The <see cref="Menu"/> currently attached to this window. Set by <c>Menu.Activate()</c>. Replaces the deprecated <c>System.Windows.Forms.MainMenu</c>.</summary>
		...
		/// <summary>Always returns 2 (top-level modeless). Overriding this prevents <see cref="Form.Show"/> from trying to set <c>MdiParent</c> on the container itself. The setter is a no-op.</summary>
		...
		// ── Forms collection ──────────────────────────────────────────────────
		/// <summary>Returns the live <see cref="Form"/> at VFP 1-based position <paramref name="i"/> in the registered-forms list. Equivalent to VFP <c>_Screen.Forms(i)</c>.</summary>
		PUBLIC METHOD Forms( i AS INT ) AS Form
			RETURN SELF:_Forms[ i - 1 ]

		/// <summary>Number of forms currently registered with this window. Equivalent to VFP <c>_Screen.FormCount</c>.</summary>
		PROPERTY FormCount AS LONG GET SELF:_Forms:Count

		// ── Registration ──────────────────────────────────────────────────────
		/// <summary>Registers <paramref name="frm"/> in the live-forms list. Called by <c>Form.OnLoad</c>; automatically wires <c>FormClosed</c> to remove the form when it closes.</summary>
		...
		/// <summary>Removes the closed form from the live-forms list. Wired automatically by <see cref="RegisterForm"/>.</summary>
		...
		/// <summary>After the window is shown, evaluates <see cref="StartFunction"/> as a macro or launches <see cref="StartForm"/> via <c>__VFPDoForm.Create</c>, matching VFP start-up behaviour.</summary>
