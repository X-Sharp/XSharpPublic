...
/*	METHOD __WriteString_unused(oWriter AS BinaryWriter , cString AS STRING , nLength AS INT) AS VOID
	...
	METHOD OpenXml(cFileName AS STRING , lFileIsXml REF LOGIC) AS LOGIC
		...
		LOCAL lNewWindow := FALSE AS LOGIC
		LOCAL lSuccess := FALSE AS LOGIC
	...
	METHOD OpenXmlItem(oBaseNode AS XmlNode) AS VOWEDItem
		...
		DO WHILE oNode != NULL
			...
			CASE cName == "NOSAVE"
				oItem:nNoSave := (BYTE) iif(cValue:ToUpper() == "YES" , 1 , 0)
			CASE cName == "DELETED"
	...
	METHOD OpenVNfrm(cFileName AS STRING) AS LOGIC
		LOCAL oReader := NULL AS BinaryReader
		LOCAL oDesign := NULL AS DesignWindowItem
		LOCAL oItem AS VOWEDItem
		LOCAL cGuid AS STRING
		LOCAL lError := FALSE AS LOGIC
		LOCAL lFileIsXml := FALSE AS LOGIC
...
//				oDesign:Deleted := (Int32)oItem:nDeleted
	...
	INTERNAL METHOD __ReadVNFrmProperties(oItem AS VOWEDItem , oDesign AS DesignWindowItem) AS VOID
...
*/
	...
	INTERNAL METHOD SaveToXml(oDocument AS XmlDocument , oDesign AS DesignWindowItem , oParentNode AS XmlNode) AS LOGIC
