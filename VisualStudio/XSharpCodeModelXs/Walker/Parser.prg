...
BEGIN NAMESPACE XSharpModel
	CLASS Parser
		...
		PRIVATE aOptionnalEndMarkers AS STRING[]
		PRIVATE aSingleOptionnalEndMarkers AS STRING[]
		...
		METHOD SetCoreDialect() AS VOID
			...
			// These are for structures
			aSpecialEndMarkers := <STRING>{"IF", "DO", "WHILE", "WITH" }
			// These are for Entities
			aOptionnalEndMarkers := <STRING>{ "METHOD", "FUNCTION", "PROCEDURE", "ACCESS", "ASSIGN" }
			aSpecialEndClassMarkers := <STRING>{}
			aSingleOptionnalEndMarkers := <STRING>{}
		...
		METHOD Parse(aLineCollection AS IList<STRING>, lIncludeLocals AS LOGIC) AS ParseResult
			...
			FOREACH cLine AS STRING IN aLineCollection
				...
				DO WHILE nChar <= nLineLen // one more than chars in line
					...
					IF ( cOldChar == '>' .AND. lFoundAlias )
						lWasAliased := TRUE
					ENDIF
					IF ( cOldChar == '-' .AND. cRealChar = '>' )
						lFoundAlias := TRUE
					ELSE
						lFoundAlias := FALSE
					ENDIF
				...
				/*IF lMustLoop
					...
					IF ( SELF:Dialect == XSharpDialect.XPP ) .AND. state:lFirstWord .AND. cUpperWord:Length>0
						...
						IF (nChar < nLineLen ) .AND. cLine[nChar]==':'
							// Persistent Visibility ?
							IF SELF:dictXPPVis:ContainsKey(cUpperWord)
								// Set it as Persistent
								xppVisibility := SELF:dictXPPVis[ cUpperWord ]
								//
								LOOP
							ELSE
								xppVisibility := ""
...
//								END IF
						...
						CASE lAllowEntityParse .AND. .NOT. lEscapedWord .AND. cChar != '.' .AND. cCharBeforeWord != '.' .AND. (cUpperWord == "VAR" .AND. state:lFirstWord)
							state:lVisFound := TRUE
							IF cUpperWord == "VAR"
								IF ( SELF:Dialect == XSharpDialect.XPP )
									// VAR is a Field
									state:lField := TRUE
									// We also have to set the Visibility....
									SELF:SetAccessModifiers( xppVisibility, REF eAccessLevel, REF eModifiers, REF lStatic, REF lPartial )
								ELSE
									state:lLocal := TRUE
						...
						CASE lAllowEntityParse .AND. .NOT. lEscapedWord .AND. state:lFirstWord .AND. cUpperWord == "USING"
							...
							eStep := ParseStep.AfterUsing
							oStatementLine:cArgument := ""
						CASE state:lFirstWord .AND. lInProperty .AND. (cUpperWord == "SET" .OR. cUpperWord == "GET") .AND. .NOT. lEscapedWord .AND. oInfo == NULL
							_SetLineType(oStatementLine, LineType.TokenIn)
							oStatementLine:cArgument := cUpperWord
							state:lIgnore := TRUE
						CASE state:lFirstWord .AND. lInEvent .AND. (cUpperWord == "ADD" .OR. cUpperWord == "REMOVE") .AND. .NOT. lEscapedWord .AND. oInfo == NULL
							_SetLineType(oStatementLine, LineType.TokenIn)
							oStatementLine:cArgument := cUpperWord
							state:lIgnore := TRUE
							...
							ELSEIF eStep == ParseStep.AfterEnd
								...
								SWITCH cUpperWord
									...
									CASE "EVENT"
										...
										lValidEndEntity := TRUE
									OTHERWISE
										IF System.Array.IndexOf(aOptionnalEndMarkers , cUpperWord) != -1
								...
								.NOT. state:lNameFound
									...
									DO CASE
										...
										CASE .NOT. state:lNameFound
											...
											END IF
											//
											IF ( oInfo:eType == EntityType._Event ) .AND. lWasAliased
											ELSE
												//
												AddEntity(oInfo, oLine)
												nRealStartLine := -1
...
//								oInfo:eType := EntityType._Event
		...
		PROTECTED METHOD SetAccessModifiers(cUpperWord AS STRING, eAccessLevel REF AccessLevel , eModifiers REF EntityModifiers, lStatic REF LOGIC, lPartial REF LOGIC) AS VOID
