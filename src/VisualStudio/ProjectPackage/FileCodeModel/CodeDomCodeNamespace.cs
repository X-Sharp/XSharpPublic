			...
			get
			{
				...
				foreach (CodeTypeDeclaration ctd in CodeObject.Types)
				{
					if (ctd.UserData[CodeKey] == null)
					{
						if (ctd.IsClass)
						{
							ctd.UserData[XSharpFileCodeModel.CodeKey] = CodeDomCodeClass.Create(DTE, this, ctd);
						}
						else if (ctd.IsInterface)
						{
							ctd.UserData[XSharpFileCodeModel.CodeKey] = new CodeDomCodeInterface(DTE, this, ctd);
						}
						else if (ctd.IsEnum)
						{
							ctd.UserData[XSharpFileCodeModel.CodeKey] = new CodeDomCodeEnum(DTE, this, ctd);
						}
						else if (ctd.IsStruct)
						{
							ctd.UserData[XSharpFileCodeModel.CodeKey] = new CodeDomCodeStruct(DTE, this, ctd);
						}
						else if (ctd is CodeTypeDelegate)
						{
							ctd.UserData[XSharpFileCodeModel.CodeKey] = new CodeDomCodeDelegate(DTE, this, (CodeTypeDelegate)ctd);
						}
					}
