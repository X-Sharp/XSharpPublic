...
oDCsleLM := SingleLineEdit{SELF,ResourceID{OMARGINDIALOG_SLELM,_GetInst()}}
oDCsleLM:HyperLabel := HyperLabel{#sleLM,NULL_STRING,NULL_STRING,NULL_STRING}

oDCsleRM := SingleLineEdit{SELF,ResourceID{OMARGINDIALOG_SLERM,_GetInst()}}
oDCsleRM:HyperLabel := HyperLabel{#sleRM,NULL_STRING,NULL_STRING,NULL_STRING}

oCCpbOK := PushButton{SELF,ResourceID{OMARGINDIALOG_PBOK,_GetInst()}}
oCCpbOK:HyperLabel := HyperLabel{#pbOK,"OK",NULL_STRING,NULL_STRING}

oCCpbCancel := PushButton{SELF,ResourceID{OMARGINDIALOG_PBCANCEL,_GetInst()}}
oCCpbCancel:HyperLabel := HyperLabel{#pbCancel,"Cancel",NULL_STRING,NULL_STRING}
...
METHOD RTFChangeFont() 
   ...
   oFontDlg := StandardFontDialog{SELF:Owner}
   oFontDlg:FontColor := SELF:TextColor
   oFontDlg:Font      := SELF:Font
   oFontDlg:Show()

   SELF:TextColor := oFontDlg:FontColor
   SELF:Font      := oFontDlg:Font
	

END CLASS
#warning The following method did not include a CLASS declaration
CLASS SaveAsDialog_external_class INHERIT SaveAsDialog
ACCESS FilterIndex 
