        ...
        private sealed class WasPropertyBackingFieldAccessChecked : BoundTreeWalkerWithStackGuardWithoutRecursionOnTheLeftOfBinaryOperator
        {
            ...
            public override BoundNode? VisitPropertyAccess(BoundPropertyAccess node)
            {
                    ...
                    !node.WasPropertyBackingFieldAccessChecked)
                {
                    _found = node;
#if XSHARP
                    if (node.PropertySymbol is XsVariableSymbol)
                    {
                        // We don't want to report backing field access for X# variables.
                        // This is because we don't have a way to distinguish between property access and variable access.
                        _found = null;
                    }
                    else
                    {
                        var syntaxNode = node.PropertySymbol.GetNonNullSyntaxNode();
                        if (syntaxNode.XGenerated)
                        {
                            // Do not generate an error when we access a property that is generated by X#.
                            _found = null;
                        }
                    }
