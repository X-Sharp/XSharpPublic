...
PARTIAL CLASS VOFieldSpecEditor INHERIT DesignerBase
	...
	INTERNAL oListView AS FSEListView
	EXPORT lLoadedAsXml AS LOGIC

	PROTECT oProject AS XSharp.ProjectAPI.Project
...
//		SELF:oListView:GotFocus += EventHandler{SELF , @GotFocusHandler()}
	...
	METHOD GetFieldSpecs() AS FSEDesignFieldSpec[]
		...
		FOREACH oItem as FSEDesignListViewItem in SELF:oListView:Items
			aRet[++n] := oItem:oDesign
		NEXT
	RETURN aRet
...
//			oDesign:PropertyValueSelected(cProp , oValue)
	...
	METHOD Save(cFileName AS STRING , lVnfrmOnly AS LOGIC) AS LOGIC
		...
		IF SELF:GetSaveFileStreams(cFileName , cPrgFileName , oPrgStream , lVnfrmOnly)

			oModule := XSharp.ProjectAPI.Projects.Find(cPrgFileName)
            ...
            IF oProject != NULL_OBJECT
			    oProject:AddItem(cFileName)

			    LOCAL cDelete AS STRING
			    FOR n := 0 UPTO SELF:aFilesToDelete:Count - 1
				    cDelete := SELF:aFilesToDelete[n]
				    // The selected .vnfs file will be removed from the Project Package
				    IF FileInfo{cDelete}:FullName:ToUpper() != FileInfo{cOrigFilename}:FullName:ToUpper()
                    
					    oProject:RemoveItem(cDelete)
	...
	PROTECTED METHOD GetSaveFileStreams(cVNfsFileName REF STRING , cPrgFileName REF STRING , ;
		...
		IF .not. SELF:lLoadedAsXml
			cVNfsFileName := Funcs.GetModuleFilenameFromBinary(cVNfsFileName) + ".Fieldspecs.vnfs"
			IF .not. VOFieldSpecEditor.lEditorCloseWarningShown
				IF .not. Funcs.QuestionBox(e"By saving the editor contents, FieldSpec definitions will be written to file: \n\n" + cVNfsFileName + ;
										 ...
										 e"The current editor will be closed and all selected .vnfs files will be removed from the project." + ;
	...
	METHOD Open(cFileName AS STRING) AS LOGIC
		LOCAL aVnfs AS IList<XSharp.ProjectAPI.VOBinary>
		LOCAL oDesign AS FSEDesignFieldSpec
		LOCAL aFieldSpecs AS ArrayList
		LOCAL lModule := FALSE AS LOGIC
		...
		TRY
			SELF:oProject := XSharp.ProjectAPI.Projects.Find(Funcs.GetModuleFilenameFromBinary(cFileName) + ".prg"):Parent
		END TRY
		IF SELF:oProject == NULL
            Funcs.WarningBox("Cannot find the project, the fieldspeceditor will not be able to locate all the fieldspecs")		
        END IF
...
//			MessageBox.Show("Fieldspec definitions read from xml file: " + cFileName , "FieldSpec Editor")
		...
		END IF
        if oProject != NULL_OBJECT
		    aVnfs := SELF:oProject:GetBinaries(XSharp.ProjectAPI.VOBinaryType.FieldSpec , FALSE)
		    FOR n := 0 UPTO aVnfs:Count - 1
			    lModule := Funcs.GetModuleFilenameFromBinary(aVnfs[n]:Name):ToUpper() == Funcs.GetModuleFilenameFromBinary(cFileName):ToUpper()
			    IF lModule
    //				MessageBox.Show(aVnfs[n]:Name)
				    IF aVnfs[n]:Name:ToUpper():Contains(".FIELDSPECS.VNFS")
					    IF .not. SELF:lLoadedAsXml
						    Funcs.WarningBox(e"Please open FieldSpec definitions via \"" + Funcs.GetModuleNameFromBinary(cFileName) + e".FieldSpecs.vnfs\" file." , "FieldSpec Editor")
						    RETURN FALSE
					    END IF
				    ELSE
					    IF SELF:lLoadedAsXml .and. !lMerge
						    IF .not. Funcs.QuestionBox("Additional FieldSpec definitions were found in module, do you want to merge them ?" , "FieldSpec Editor")
							    EXIT
						    ELSE
							    lMerge := TRUE
						    END IF
					    END IF
					    oDesign := VOFieldSpecEditor.OpenVNfs(aVnfs[n]:Name , SELF)
					    IF oDesign != NULL
						    aFieldSpecs:Add(oDesign)
						    SELF:aFilesToDelete:Add(aVnfs[n]:Name)
					    END IF
				    END IF
			    END IF
		    NEXT
...
//		VOFieldSpecEditor.Template:Read(SELF:cLoadedDir)
	...
	STATIC METHOD GetCodeContents(oDesign AS FSEDesignFieldSpec) AS CodeContents
		...
		oCode := CodeContents{}
		aClass := oCode:aClass
		FOREACH cLine as STRING IN VOFieldSpecEditor.Template:aClass
			
			aClass:Add(TranslateLine(cLine , oDesign))
		NEXT
		
		aConstructor := oCode:aConstructor
		FOREACH cLine as STRING IN VOFieldSpecEditor.Template:aInit
			aConstructor:Add(TranslateLine(cLine , oDesign))
		NEXT
...
INTERNAL CLASS FSEListView INHERIT ListView
...
/*		IF SELF:Items:Count == 0 .and. SELF:oItemEdit == NULL
	...
	METHOD ContextMenuPopUp(o AS OBJECT,e AS EventArgs) AS VOID
		LOCAL oDesign := NULL AS FSEDesignFieldSpec
...
CLASS FieldSpecCode
	...
	METHOD Read(cDirectory AS STRING) AS LOGIC
		...
		LOCAL aRead := NULL AS List<STRING>
...
INTERNAL CLASS FSETextBox INHERIT TextBox
