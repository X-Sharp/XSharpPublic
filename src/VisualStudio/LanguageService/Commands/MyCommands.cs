...
namespace Community.VisualStudio.Toolkit
{
    ...
    public class MyCommands
    {
        ...
        public async Task<IDisposable> InterceptAsync(CommandID cmd, Func<CommandProgression> func)
        {
            ...
            MyCommandInterceptor interceptor = new MyCommandInterceptor(cmd, func);

            ErrorHandler.ThrowOnFailure(priority.RegisterPriorityCommandTarget(0, interceptor, out uint cookie));

            return new MyDisposable(() => priority.UnregisterPriorityCommandTarget(cookie));
        }
    }
    ...
    internal sealed class MyDisposable : IDisposable
    {
        ...
        public MyDisposable(Action action)
        {
            _action = action;
        }
    ...
    internal class MyCommandInterceptor : IOleCommandTarget
    {
        private readonly CommandID _cmd;
        private readonly Func<CommandProgression> _func;

        public MyCommandInterceptor(CommandID cmd, Func<CommandProgression> func)
        {
            _cmd = cmd;
            _func = func;
        }
