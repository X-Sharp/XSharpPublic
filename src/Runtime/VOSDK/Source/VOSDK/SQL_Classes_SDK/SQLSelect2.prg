...
#ENDIF
    ...
    IF !oInsert:Execute()
        SELF:__CopySQLError( __FUNCTION__, oInsert:ErrInfo )
    ...
    IF nRowCount = 0
        oStmt:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__NO_INS ), __FUNCTION__ )
        RETURN FALSE
    ENDIF
...
#ENDIF
    ...
    IF nIndex = 0 .OR. nIndex > nNumCols
        oStmt:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__BADCOL ), __FUNCTION__ )
    ELSE
...
#ENDIF
    ...
    IF ! lCsrOpenFlag
        oStmt:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__NO_CSR ), __FUNCTION__ )
        RETURN FALSE
    ENDIF
    ...
    IF !oDelete:Execute()
        SELF:__CopySQLError( __FUNCTION__, oDelete:ErrInfo )
        IF oStmt:__ErrInfo:ReturnCode != SQL_SUCCESS_WITH_INFO
