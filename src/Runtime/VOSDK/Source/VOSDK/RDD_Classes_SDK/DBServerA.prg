...
#ENDIF
    ...
    IF !IsNumeric(newMode)
        oErrorInfo:=DbError{ SELF, __FUNCTION__, EG_ARG, __CavoStr(__CAVOSTR_DBFCLASS_BADCONCURRENCYASSIGN), nMode, "nMode" }
...
#ENDIF
    IF ALen(aStruct) == 0
        SELF:Error( __MakeErrObj(0), __FUNCTION__ )
    ENDIF
...
#ENDIF
    ...
    BEGIN SEQUENCE
        VoDbSelect( wWorkArea, OUT dwCurrentWorkArea )
        IF ! VoDbOrderInfo(DBOI_INDEXEXT, "", NIL, REF uOrdVal)
            BREAK ErrorBuild(_VoDbErrInfoPtr())
        ENDIF
