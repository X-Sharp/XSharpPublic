...
RETURN
	...
	METHOD GetCodeContents() AS CodeContents
		...
		LOCAL aDesign := NULL AS ArrayList
		LOCAL aColumns := NULL AS ArrayList
		LOCAL oDesign AS DesignWindowItem
		LOCAL cTemp AS STRING
		LOCAL lPixPos := FALSE AS LOGIC
		LOCAL lExpCtls := FALSE AS LOGIC
		LOCAL oRect AS Rectangle
		LOCAL oWinRect AS Rectangle
		LOCAL oProp AS VODesignProperty
		LOCAL lHasServer := FALSE AS LOGIC
...
//		LOCAL rDivX , rDivY AS REAL8
		...
		LOCAL lFontUsed := FALSE AS LOGIC
...
/*						IF oProp:cSpecialClass != NULL
	...
	INTERNAL METHOD GetDesignerUnderPoint(_oPoint AS Point) AS DesignWindowItem
		LOCAL oDesign AS DesignWindowItem
		LOCAL oRet := NULL AS DesignWindowItem
		LOCAL aDesign AS ArrayList
	...
	METHOD Open() AS VOID
		...
		oDialog:Filter := "Transported WED files (*.??frm)|*.??frm"
...
//		SELF:OpenVNfrm("C:\Documents and Settings\Cpc\My Documents\Visual Studio Projects\TestProject\TestWin\Resources\TabWin.TabWin.vnfrm")
	...
	METHOD Save() AS VOID
		...
		ENDIF
		oDlg := SaveFileDialog{}
		oDlg:Filter := "Transported WED files (*.??frm)|*.??frm"
		IF oDlg:ShowDialog() == DialogResult.OK
	...
	METHOD Save(cFileName AS STRING , lVnfrmOnly AS LOGIC) AS LOGIC
		...
		LOCAL oCode AS CodeContents
		LOCAL lRcInSameFolder := FALSE AS LOGIC
...
//			SELF:SaveVNfrm(oVNFrmStream)
	...
	PROTECTED METHOD GetSaveFileStreams(cVNFrmFileName AS STRING , oVNFrmStream REF FileStream , ;
		...
		TRY
			...
			nAt := cBaseName:ToLower():IndexOf(".vnfrm")
            if nAt == -1
...
//			MessageBox.Show(cVNFrmFileName + Chr(13)+ Chr(13) + cRCFileName + Chr(13)+ Chr(13) + cPrgFileName , (File.Exists(cVNFrmFileName) .and. File.Exists(cRCFileName) .and. File.Exists(cPrgFileName)):ToString())
	...
	INTERNAL VIRTUAL METHOD CreateControl(cControl AS STRING, oParent AS Control , cGuid AS STRING) AS DesignWindowItem
...
//		SELF:oSurface:Parent:Text := cText
	...
	INTERNAL VIRTUAL METHOD GetDesignItemFromControl(oControl AS Control) AS DesignWindowItem
		IF oControl == NULL
			RETURN NULL
		ENDIF
	...
	INTERNAL VIRTUAL METHOD GoniaClicked(oSelector AS SelectorBitmap , e AS MouseEventArgs) AS VOID
		IF SELF:lReadOnly
			RETURN
		ENDIF
//		SELF:oDummy:Focus()
	...
	INTERNAL VIRTUAL METHOD ControlMouseDoubleClick(oItem AS DesignWindowItem , eButton AS MouseButtons , oPoint AS Point) AS VOID
	...
	INTERNAL VIRTUAL METHOD ControlMouseDown(oItem AS DesignWindowItem , eButton AS MouseButtons , oPoint AS Point) AS VOID
...
//						nPage := oPoint:X / 60
		...
		END CASE
	RETURN
	INTERNAL VIRTUAL METHOD ControlMouseMove(oItem AS DesignWindowItem , eButton AS MouseButtons , oPoint AS Point) AS VOID
		LOCAL oTemp AS Point
...
/*		CASE SELF:eCurrent == WEDAction.Resize
	...
	RETURN
	INTERNAL VIRTUAL METHOD ControlMouseUp(oItem AS DesignWindowItem , e AS MouseButtons , oPoint AS Point) AS VOID
		LOCAL oRect AS Rectangle
		LOCAL n,m AS INT
...
//			IF SELF:oActionEnd != SELF:oActionStart
			IF SELF:lMoved
				SELF:DrawFrame()
				IF SELF:ViewMode == ViewMode.Browse
					LOCAL oTargetColumn := NULL AS DesignWindowItem
					LOCAL nTargetIndex := 0 AS INT
					LOCAL aNewOrder AS ArrayList
					LOCAL aSorted AS ArrayList
					LOCAL lLeft := FALSE AS LOGIC
...
/*		LOCAL lGrid AS LOGIC
	...
	VIRTUAL METHOD EndAction() AS VOID
		LOCAL oDesign := NULL AS DesignWindowItem
		LOCAL oAction := NULL AS DesignerBasicAction
...
//		LOCAL lInclude AS LOGIC
	...
	RETURN
	INTERNAL VIRTUAL METHOD CopyDesigner(oDesign AS DesignWindowItem , cParent AS STRING) AS VOID
	...
	INTERNAL  VIRTUAL METHOD ShowContextMenu(oDesign AS DesignWindowItem , oPoint AS Point) AS VOID
...
/*			IF oDesign:IsBrowser
	...
	INTERNAL VIRTUAL METHOD PropertyGotUpdated(oDesign AS DesignWindowItem , oProp AS VODesignProperty) AS VOID
		SELF:ApplyProperty(oDesign , oProp)
	RETURN
	INTERNAL VIRTUAL METHOD ApplyProperty(oDesign AS DesignWindowItem , oProp AS VODesignProperty) AS VOID
	...
	INTERNAL VIRTUAL METHOD AddAffected(oDesign AS DesignWindowItem) AS VOID
		IF !SELF:aAffected:Contains(oDesign:cGuid)
			SELF:aAffected:Add(oDesign:cGuid)
		ENDIF
	RETURN
	
	INTERNAL VIRTUAL METHOD GetDesignItemFromGuid(cGuid AS STRING) AS DesignWindowItem
		LOCAL oDesign AS DesignWindowItem
		LOCAL aItems AS ArrayList
		LOCAL n AS INT
	...
	ACCESS Module AS XSharp.ProjectAPI.Module
		LOCAL cPrgFileName AS STRING
		cPrgFileName := Funcs.GetModuleFilenameFromBinary(SELF:cDefaultFileName) + ".prg"
	RETURN XSharp.ProjectAPI.Projects.Find(cPrgFileName)

	INTERNAL METHOD WriteCallback(oDesign AS DesignWindowItem , cName AS STRING) AS VOID
		LOCAL oProject AS XSharp.ProjectAPI.Project
        ...
        ELSE
		    oMember := oProject:FindMember(SELF:oWindowDesign:Name, cName,  FALSE)
		...
		nLine := oEditor:FindEntityLine(cName , cClass , EntityType._Method)
		IF nLine != 0
			oModule:OpenInEditor(nLine, 1)
			RETURN
		END IF
		*/
		...
		TRY
			FOR n := 0 UPTO aLines:Count - 1
				oModule:InsertLine(nLine + n , SubstituteTpl(aLines[n] , cClass , SELF:oWindowDesign:cInitMethod) + e"\r\n")
			NEXT
		END TRY
	...
	METHOD TestForm() AS VOID
		...
		FOR n := 0 UPTO aDesign:Count - 1
			...
			TRY
                ...
                if oControl IS ITestableControl
                    LOCAL oITest as ITestableControl
                    oITest := (ITestableControl) oControl
                    oITest:lTestMode := TRUE
                ENDIF
...
INTERNAL CLASS DesignWindowItem INHERIT DesignItem
...
//		oItem:cControl := SELF:cControl
		...
		oItem:nNoSave := (BYTE) iif(SELF:GetProperty("_GenCode"):TextValue:ToUpper() == "NO" , 1 , 0)
