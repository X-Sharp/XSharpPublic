...
INTERNAL CLASS SelectorBitmap INHERIT PictureBox
	EXPORT nGonia AS INT
	INTERNAL oDesign AS DesignWindowItem
	INTERNAL CONSTRUCTOR(_oDesign AS DesignWindowItem , _nGonia AS INT)
		SUPER()
...
CLASS EditorStream
	...
	METHOD Load(cFileName AS STRING) AS VOID
		LOCAL aLines AS System.Collections.Generic.List<STRING>
		...
		TRY
			...
			ELSE
				...
				oReader := System.IO.StreamReader{oStream , oEncoding}
				aLines := System.Collections.Generic.List<STRING>{100}
...
INTERNAL INTERFACE ITestableControl
    PROPERTY lTestMode as LOGIC GET SET
    PROPERTY oItem AS DesignWindowItem GET SET
END INTERFACE
