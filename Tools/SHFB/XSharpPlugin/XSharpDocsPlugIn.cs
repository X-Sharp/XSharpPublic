    ...
    public sealed class XSharpDocsPlugIn : IPlugIn
    {
            ...
            get
            {
                if (executionPoints == null)
                    executionPoints = new List<ExecutionPoint>
                    {
                        ...
                        new ExecutionPoint(BuildStep.GenerateFullTextIndex, ExecutionBehaviors.After,10000)
        ...
        public void Execute(ExecutionContext context)
        {
            ...
            if (context.BuildStep == BuildStep.GenerateFullTextIndex)
            {
                ...
                var elements = webtoc.XPathSelectElements("//HelpTOCNode[contains(@Url,\"Functions.htm\") and (contains(@Url, \"Methods\") or contains(@Url, \"Fields\"))]");

                foreach (var element in elements)
                {
                    var parent = (XElement) element.Parent;
                    parent.SetAttributeValue("Title", "Functions, Globals and Defines");
                    parent.ReplaceWith(parent);
                    var attr = element.Attribute("Title");
                    var title = attr.Value.Trim();
                    if (title.EndsWith("Methods"))
                    {
                        element.SetAttributeValue("Title", "List of Functions");
                        element.ReplaceWith(element);
                        var child = (XElement) element.FirstNode;
                        ReplaceNodes(child, "Method", "Function");
                    }
                    else if (title.EndsWith("Fields"))
                    {
                        element.SetAttributeValue("Title", "List of Globals and Defines");
                        element.ReplaceWith(element);
                        var child = (XElement)element.FirstNode;
                        ReplaceNodes(child, "Field", "");
                    }
                }
                webtoc.Save(file);
                return;
        ...
        void ReplaceNodes(XElement child, string cSearch, string cReplace)
        {
            while (child != null)
            {
                var attr = child.Attribute("Title");
                var title = attr.Value.Trim();
                title = title.Replace(cSearch, cReplace);
                child.SetAttributeValue("Title", title);
                child.ReplaceWith(child);
                if (child.FirstNode != null)
                {
                    ReplaceNodes((XElement)child.FirstNode, cSearch, cReplace);
                }
