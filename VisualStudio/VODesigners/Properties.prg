		...
		IF SELF:cSpecialClass != NULL
			...
			CASE SELF:oValue:GetType() == TypeOf(MenuAccelerator)
				...
				IF oAccel:IsEmpty
					RETURN ""
				END IF
				cValue := ((Char) ((DWORD)(48 + iif(oAccel:Control , 4 , 0) + iif(oAccel:Shift , 2 , 0) + iif(oAccel:Alt , 1 , 0)))):ToString()
				cValue += oAccel:Key
				RETURN cValue
			END CASE
		ENDIF
...
STATIC CLASS VODefines
	STATIC PRIVATE aDefines AS System.Collections.Generic.Dictionary<STRING, DWORD>
	STATIC CONSTRUCTOR()
		VODefines.aDefines := System.Collections.Generic.Dictionary<STRING, DWORD>{}
