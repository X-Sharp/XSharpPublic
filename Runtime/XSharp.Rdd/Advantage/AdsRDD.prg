...
CLASS XSharp.ADS.AdsRDD INHERIT Workarea
    ...
    PRIVATE METHOD _CheckVODeletedFlag() AS DWORD
        RETURN ACE.AdsShowDeleted(IIF(RuntimeState.Deleted,(WORD)0 ,(WORD)1 ))
        
    PRIVATE METHOD _CheckVODateFormat() AS LOGIC
    ...
    PRIVATE METHOD _Ansi2Unicode(source AS CHAR[], iLength AS LONG) AS STRING
        TRY
    ...
    METHOD _FieldSub() AS LOGIC
        ...
        FOR num := 1u TO wFields STEP 1
            ...
            SELF:ACECALL(ACE.AdsGetFieldName(SELF:_Table, (WORD)num, sb, REF pusBufLen))
    ...
    PRIVATE METHOD _SetPaths() AS DWORD
    ...
    VIRTUAL METHOD Seek(seekinfo AS DBSEEKINFO) AS LOGIC
        ...
        IF seekInfo:Last
            ...
            IF found== 0 .AND. seekinfo:SoftSeek  
                SELF:ACECALL(ACE.AdsSeek(SELF:_Index, Key, (WORD)Key:Length , ACE.ADS_STRINGKEY, seekMode, OUT found))
            ENDIF
        ELSE
            SELF:ACECALL(ACE.AdsSeek(SELF:_Index, Key, (WORD)Key:Length , ACE.ADS_STRINGKEY, seekMode, OUT found))
        ENDIF
        ...
        VAR result := ACE.AdsGetRecordNum(SELF:_Table, ACE.ADS_IGNOREFILTERS, OUT dwRecord)
        IF result == ACE.AE_NO_CURRENT_RECORD
            SELF:_Recno := 0
        ELSEIF result == 0
            SELF:_Recno := dwRecord
        ELSE
            SELF:ACECALL(result)
        ENDIF
        RETURN TRUE
        
    PRIVATE METHOD _SetScope(hIndex AS System.IntPtr, usScopeOption AS WORD, oScope AS OBJECT ) AS OBJECT
    ...
    METHOD GetValue(nFldPos AS INT) AS OBJECT
        ...
        CASE DbFieldType.Date
                ...
                result := ACE.AdsGetJulian(SELF:_Table, dwField, OUT julianDate)
                IF result ==  ACE.AE_NO_CURRENT_RECORD
    ...
    METHOD GetValueLength(nFldPos AS LONG) AS LONG
        ...
        IF fld:FieldType == DbFieldType.Memo
            SELF:ACECALL(ACE.AdsGetFieldLength(SELF:_Table, dwField,OUT dwLen))
            RETURN (LONG) dwLen
    ...
    VIRTUAL METHOD Info(uiOrdinal AS LONG, oNewValue AS OBJECT) AS OBJECT
        ...
        CASE DbInfo.DBI_GETHEADERSIZE
            ...
            ACE.AdsSetProperty90(SELF:_Table, ACE.ADS_GET_HEADER_LENGTH, REF dwValue)
            RETURN (LONG) dwValue
    ...
    VIRTUAL METHOD CreateMemFile() AS LOGIC
    ...
    VIRTUAL METHOD HeaderLock(uiMode AS DbLockMode) AS LOGIC  
        ...
        /// <inheritdoc />
	VIRTUAL METHOD OpenMemFile(fileName AS STRING ) AS LOGIC
...
#region Index and Order operations
	VIRTUAL METHOD OrderListRebuild() AS LOGIC
        LOCAL options AS DWORD
		SELF:ACECALL(ACE.AdsGetTableOpenOptions(SELF:_Table, OUT options))
		IF !_HasFlag(options, ACE.ADS_EXCLUSIVE)
			SELF:ADSERROR(ACE.AE_TABLE_NOT_EXCLUSIVE, EG_SHARED, "OrderListRebuild")
			RETURN FALSE
		ENDIF
		SELF:ACECALL(ACE.AdsReindex(SELF:_Table))
		RETURN SELF:GoTop()
        
	VIRTUAL METHOD OrderListDelete(info AS DBORDERINFO ) AS LOGIC
		SELF:ACECALL(ACE.AdsCloseAllIndexes(SELF:_Table))
		SELF:_Index := IntPtr.Zero
		RETURN TRUE
	...
	VIRTUAL METHOD OrderDestroy(info AS DBORDERINFO ) AS LOGIC
		...
		IF info:Order IS STRING
			...
			SELF:ACECALL(ACE.AdsGetIndexHandle(SELF:_Table, orderName, OUT hIndex))
		ELSE
			TRY
				orderNum := Convert.ToUInt16(info:Order)
			CATCH
				SELF:ADSERROR(ERDD_DATATYPE, EG_DATATYPE, "OrderDestroy")
				RETURN FALSE
			END TRY
			IF orderNum > 0
                SELF:ACECALL(ACE.AdsGetIndexHandleByOrder(SELF:_Table, orderNum, OUT hIndex))
			ENDIF
		ENDIF
		IF hIndex != IntPtr.Zero
			SELF:ACECALL(ACE.AdsDeleteIndex(hIndex))
			IF hIndex == SELF:_Index
				SELF:_Index := IntPtr.Zero
			ENDIF
		ENDIF
		RETURN TRUE
	...
	VIRTUAL METHOD OrderListAdd(info AS DBORDERINFO  ) AS LOGIC
		...
		IF (SELF:_SetPaths() != 0)
			RETURN FALSE
		ENDIF
		SELF:_CheckVODateFormat()
		SELF:ACECALL(ACE.AdsGetNumIndexes(SELF:_Table, OUT numIndexes))
		wLength := 1000
		indices := IntPtr[]{ wLength }
        cIndexName := info:BagName
		result := ACE.AdsOpenIndex(SELF:_Table, cIndexName, indices, REF wLength)
		IF result != ACE.AE_INDEX_ALREADY_OPEN 
			SELF:ACECALL(result)
			IF SELF:_Index == IntPtr.Zero .AND. wLength > 0
				wCurrent := numIndexes +1
				SELF:ACECALL(ACE.AdsGetIndexHandleByOrder(SELF:_Table, wCurrent, OUT SELF:_Index))
			ENDIF
		ELSE
	...
	VIRTUAL METHOD OrderListFocus(info AS DBORDERINFO ) AS LOGIC
		...
		ELSEIF info:Order IS STRING
			...
			IF ! String.IsNullOrEmpty(orderName)
				orderName := Path.GetFileNameWithoutExtension(orderName)
				SELF:ACECALL(ACE.AdsGetIndexHandle(SELF:_Table, orderName, OUT SELF:_Index))
				RETURN SELF:GoTop()
			ENDIF
			SELF:_Index := IntPtr.Zero
        ELSE
			...
			TRY
				orderNum := Convert.ToUInt16(info:Order)
			CATCH
				SELF:ADSERROR(ERDD_DATATYPE, EG_DATATYPE, "OderListFocus")
			END TRY
			IF orderNum > 0
				SELF:ACECALL(ACE.AdsGetIndexHandleByOrder(SELF:_Table, orderNum, OUT SELF:_Index))
				RETURN SELF:GoTop()
            ELSE
                SELF:_Index := IntPtr.Zero
			ENDIF
	...
	VIRTUAL METHOD OrderCreate(info AS DBORDERCREATEINFO ) AS LOGIC
		...
		IF (info:Unique)
			mode |= ACE.ADS_UNIQUE
		ENDIF
		IF (SELF:_OrderCondInfo != NULL)
			cCondition := SELF:_OrderCondInfo:ForExpression
			IF SELF:_OrderCondInfo:Custom
				mode |= ACE.ADS_CUSTOM
			ENDIF
			IF SELF:_OrderCondInfo:Descending
				mode |= ACE.ADS_DESCENDING
			ENDIF
			nonAdditive := SELF:_OrderCondInfo:Additive
			mustEval    := SELF:_OrderCondInfo:EvalBlock != NULL
		ENDIF
		IF (nonAdditive)
		...
		IF info:Order IS STRING 
			...
			result := ACE.AdsCreateIndex90(SELF:_Table, info:BagName, cTag, info:Expression, cCondition, NULL, mode, 0, SELF:_Collation, OUT SELF:_Index)
		ELSE
			IF (SELF:_TableType == ACE.ADS_NTX)
				result := ACE.AdsCreateIndex(SELF:_Table, info:BagName, NULL, info:Expression, cCondition, NULL, mode, OUT SELF:_Index)
			ELSE
				fileNameWithoutExtension := Path.GetFileNameWithoutExtension(info:BagName)
				mode |= ACE.ADS_COMPOUND
				result := ACE.AdsCreateIndex90(SELF:_Table, info:BagName, fileNameWithoutExtension, info:Expression, cCondition, NULL, mode, 0u, SELF:_Collation, OUT SELF:_Index)
			ENDIF
		ENDIF
		IF (mustEval)
    ...
    PRIVATE METHOD ProgressCallback(usPercentDone AS WORD , ulCallbackID AS DWORD ) AS DWORD
		IF (usPercentDone > 100)
			SELF:_iProgress := 100
		ELSE
			SELF:_iProgress := usPercentDone
		ENDIF
		IF SELF:_OrderCondInfo != NULL .AND. SELF:_OrderCondInfo:EvalBlock != NULL
			SELF:_OrderCondInfo:EvalBlock:EvalBlock(SELF:_iProgress)
		ENDIF
		RETURN 0u
	...
	VIRTUAL METHOD OrderInfo(nOrdinal AS DWORD, info AS DbOrderInfo) AS OBJECT
		...
		IF (info:Order != NULL)
			IF info:Order IS STRING
                ...
                IF String.IsNullOrEmpty(cOrder)
                    result := 0
                ELSE
                    result := ACE.AdsGetIndexHandle(SELF:_Table, cOrder, OUT hIndex)
                ENDIF
			ELSE
				TRY
					orderNum := Convert.ToUInt16(info:Order)
                CATCH
                    orderNum := 0
					SELF:ADSERROR(ERDD_DATATYPE, EG_DATATYPE, "OrderInfo")
				END TRY
				result := ACE.AdsGetIndexHandleByOrder(SELF:_Table, orderNum, OUT hIndex)
			ENDIF
			IF (result != 0)
				hIndex := IntPtr.Zero
			ENDIF
		...
		CASE DBOI_KEYCOUNT
			...
			ELSE
                ...
                len := 0
				result := ACE.AdsGetAOFOptLevel(SELF:_Table, OUT wOptLevel, NULL, REF len)
				IF result == ACE.AE_NO_FILTER
				...
				ELSE
					SELF:ACECALL(ACE.AdsGetRecordCount(SELF:_Table, ACE.ADS_RESPECTFILTERS, OUT dwCount))
				ENDIF
				info:Result := dwCount
            ENDIF
		...
		CASE DBOI_KEYTYPE
			...
			OTHERWISE
				SELF:ADSERROR(ERDD_UNSUPPORTED, EG_UNKNOWN, "OrderInfo")
			END SWITCH
        CASE DBOI_KEYVAL
            len  := SELF:_MaxKeySize + 1
			chars := CHAR[]{len}
		...
		CASE DBOI_ORDERCOUNT
			IF !string.IsNullOrEmpty(info:BagName)
				...
				SELF:ACECALL(ACE.AdsGetAllIndexes(SELF:_Table, indices, REF numindices))
				VAR count := 0L
				...
				info:Result := count
			ELSE
				SELF:ACECALL(ACE.AdsGetNumIndexes(SELF:_Table, OUT len))
				info:Result  := (LONG)len
			ENDIF
		CASE DBOI_POSITION
        CASE DBOI_RECNO
        ...
        CASE DBOI_SCOPEBOTTOM
			...
			IF (info:Result != NULL)
				RETURN SELF:_SetScope(hIndex, wScopeOption, info:Result)
            ENDIF
            len := SELF:_MaxKeySize +1
			chars := CHAR[]{ SELF:_MaxKeySize +1}
			result := ACE.AdsGetScope(hIndex, wScopeOption, chars, REF len)
			IF result != ACE.AE_NO_SCOPE .AND. result != 0
				SELF:ACECALL(result)
            ENDIF
			info:Result := SELF:_Ansi2Unicode(chars, len)
		...
		OTHERWISE
			SELF:ADSERROR(ERDD_UNSUPPORTED, EG_UNSUPPORTED, "OrderInfo")
			RETURN FALSE
		END SWITCH
		RETURN TRUE
