...
BEGIN NAMESPACE XSharp.RDD.CDX
    ...
    INTERNAL SEALED CLASS CdxAction
        ...
        INTERNAL PageNo  := MISSING_PAGE AS DWORD
        INTERNAL PageNo2 := MISSING_PAGE AS DWORD
        INTERNAL Pos   	 := -1  AS LONG
        INTERNAL Recno 	 := MISSING_RECNO  AS DWORD
        INTERNAL Key   	 := NULL AS BYTE[]
        INTERNAL ChildPage := MISSING_PAGE AS DWORD
        ...
        INTERNAL STATIC METHOD AddKey( nRecno AS DWORD, bKey AS BYTE[]) AS CdxAction
        ...
        INTERNAL STATIC METHOD AddLeaf(oLeaf AS CdxTreePage, nRecno AS DWORD, bKey AS BYTE[] ) AS CdxAction
            RETURN CdxAction{CdxActionType.AddLeaf}{ PageNo := oLeaf:PageNo, Recno := nRecno, Key := bKey, Pos := -1}

        INTERNAL STATIC METHOD SplitLeaf(oLeaf AS CdxTreePage, nRecno AS DWORD, bKey AS BYTE[], nPos AS LONG ) AS CdxAction
            RETURN CdxAction{CdxActionType.AddLeaf}{ PageNo := oLeaf:PageNo, Recno := nRecno, Key := bKey, Pos := nPos}

        INTERNAL STATIC METHOD InsertKey(oPage AS CdxTreePage, nPos AS LONG, nRecno AS DWORD, bKey AS BYTE[]) AS CdxAction
        ...
        INTERNAL STATIC METHOD AddBranch(oPage AS CdxTreePage,  nChild AS DWORD, nRecno AS DWORD, bKey AS BYTE[]) AS CdxAction
            RETURN CdxAction{CdxActionType.AddBranch}{PageNo := oPage:PageNo, ChildPage := nChild, Recno := nRecno, Key := bKey, Pos := -1}

        INTERNAL STATIC METHOD SplitBranch(oPage AS CdxTreePage,  nChild AS DWORD, nRecno AS DWORD, bKey AS BYTE[], nPos AS INT) AS CdxAction
        ...
        INTERNAL STATIC METHOD ExpandRecnos(oPage AS CdxLeafPage, nRecno AS DWORD, bKey AS BYTE[], nPos AS INT) AS CdxAction
