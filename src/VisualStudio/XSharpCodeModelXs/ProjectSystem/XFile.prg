...
#region Fields
    ...
    PRIVATE _usings			AS SynchronizedCollection<XSourceUsing>
    ...
    CONSTRUCTOR(fullPath AS STRING, project AS XProject)
        ...
        SELF:_usings		:= SynchronizedCollection<XSourceUsing>{}
    ...
    METHOD SetTypes(types AS IDictionary<STRING, XSourceTypeSymbol>, usings AS IList<XSourceUsing>, ;
            aEntities AS IList<XSourceEntity>) AS VOID
        IF SELF:HasCode
...
#region Properties
    ...
    PROPERTY AllUsingStatics AS IList<STRING>
        GET
            ...
            foreach var us in SELF:_usings
                if us:IsStatic
                    statics:Add(us:Name)
                endif
            next
    ...
    PROPERTY UsingObjects as IList<XSourceUsing>
        GET
            RETURN SELF:_usings
        END GET
    END PROPERTY
    PROPERTY Usings AS IList<STRING>
        GET
            ...
            foreach var u in SELF:_usings
                if ! u:Modifiers:HasFlag(Modifiers.Static)
                    result:Add(u:Name)
                endif
            next
            return result
