...
namespace XSharp.Project.FileCodeModel
{
		...
		where CodeTypeType : CodeTypeDeclaration
	{
		...
		private bool IsExternalDeclared(CodeTypeMember member)
		{
            // FAB
            /*
			var vulcanMember = member.UserData["Member"] as Vulcan.Compiler.Parsetree.MemberBase;
			var projectItem = this.ProjectItem.Object as Microsoft.VisualStudio.Project.Automation.OAVSProjectItem;

            if (vulcanMember != null && projectItem != null && projectItem.FileNode != null)
                return string.Compare(vulcanMember.Location.File, projectItem.FileNode.Url, StringComparison.InvariantCultureIgnoreCase) != 0;
			else*/
				return false;
		}
		...
		public CodeElements Members
		{
			...
			get
			{
				...
				{
					// Does it belongs to the current File ?
					if (!IsExternalDeclared(member))
					{
						if (member.UserData[CodeKey] == null)
						{
                            ...
                            {
                                CodeConstructor cmm = member as CodeConstructor;
                                member.UserData[CodeKey] = new CodeDomCodeFunction(this, cmm, vsCMFunction.vsCMFunctionConstructor);
                            }
                            else if (member is CodeMemberMethod)
                            {
                                CodeMemberMethod cmm = member as CodeMemberMethod;
                                member.UserData[CodeKey] = new CodeDomCodeFunction(this, cmm, vsCMFunction.vsCMFunctionFunction);
                            }
