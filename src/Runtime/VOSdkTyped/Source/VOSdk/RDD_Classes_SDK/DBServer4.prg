...
METHOD Replace( acbExpression, aFieldList, cbForBlock, cbWhileBlock, uScope ) AS LOGIC CLIPPER
    ...
    BEGIN SEQUENCE
        ...
        IF wExprCount > 1 .AND. wExprCount != wFieldCount
            BREAK DbError{ SELF, __FUNCTION__, EG_ARG, __CavoStr( __CAVOSTR_DBFCLASS_MISMATCH ) }
        ENDIF
        // Per documentation:
    ...
    BEGIN SEQUENCE
        ...
        IF SELF:Notify( NOTIFYINTENTTOMOVE )
            IF lSelectionActive
                ...
                ELSE
                    SELF:__SetStatusHL( __FUNCTION__, EG_BOUND,  ;
...
METHOD SetDataField( nFieldPosition AS DWORD, oDataField AS DataField) AS LOGIC
    ...
    BEGIN SEQUENCE
        IF aDataFields == NULL_ARRAY
            BREAK DbError{ SELF, __FUNCTION__, EG_SEQUENCE,  __CavoStr( __CAVOSTR_DBFCLASS_NODATAFIELDSEXIST ) }


        ELSEIF  oDataField == NULL
            BREAK DbError{ SELF, __FUNCTION__, EG_ARG,  __CavoStr( __CAVOSTR_DBFCLASS_BADFIELDPOSITION ), oDataField, "oDataField" }
...
METHOD SetFilter( cbFilterBlock , cFilterText ) AS LOGIC CLIPPER
    ...
    BEGIN SEQUENCE
        ...
        ELSEIF IsNil( cbFilterBlock ) .AND. IsString(cFilterText)
            ...
            ENDIF
        ELSE
            oErr := DbError{ SELF, __FUNCTION__, EG_ARG,  ;
...
METHOD SetIndex( cIndexFileName := "" AS STRING ) AS LOGIC
    ...
    BEGIN SEQUENCE
        ...
        ELSE
            lRetCode:=FALSE
            SELF:__SetStatusHL ( __FUNCTION__, __CavoStr(__CAVOSTR_DBFCLASS_INTENTTOMOVE_CAPTION), __CavoStr(__CAVOSTR_DBFCLASS_INTENTTOMOVE) )
        ENDIF
        __DBSSetSelect(dwCurrentWorkArea)
    ...
    BEGIN SEQUENCE
        ...
        ELSE
            ...
            ELSEIF IsArray( uRelation )
                ...
                IF wFieldCount < 1
                    BREAK DbError{ SELF, __FUNCTION__, EG_ARG, __CavoStr(__CAVOSTR_DBFCLASS_NOFIELDS), uRelation, "uRelation" }
                ELSE
