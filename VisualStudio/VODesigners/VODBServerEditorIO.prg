...
//		oDesign:GetProperty("dbfname"):Value := SELF:__ReadString(oReader , 260)
		...
		aFiles := Directory.GetFiles(cBaseDir , cBaseName + "_*.vnfld")
...
//			MessageBox.Show(nPos:ToString())
		...
		aFiles := Directory.GetFiles(cBaseDir , cBaseName + "_*.vnind")
		FOR n := 1 UPTO aFiles:Length
//			MessageBox.Show(aFiles[n])
			SELF:aFilesToDelete:Add(aFiles[n])
			oReader := BinaryReader{File.Open(aFiles[n] , FileMode.Open , FileAccess.Read) , System.Text.Encoding.GetEncoding(0)}
...
/*			cBaseName := aFiles[n]
			...
			oDesign:oItem:SetValues()
			aOrders := Directory.GetFiles(cBaseDir , cBaseIndexName + "_*.vnord")
...
/*	
	STATIC METHOD FindFieldSpecInXmlFiles_unused(cModule AS STRING , cName AS STRING) AS XmlNode
		...
		IF File.Exists(cModule + ".FieldSpecs.vnfs")
			oXmlNode := FindFieldSpecInXmlFile(cModule + ".FieldSpecs.vnfs" , cName)
			IF oXmlNode != NULL
				RETURN oXmlNode
			END IF
		END IF
		...
		aFiles := Directory.GetFiles(cBaseDir , "*.FieldSpecs.vnfs")
		FOR n := 1 UPTO aFiles:Length
...
//		LOCAL n AS INT
		...
		cName := cName:ToUpper()
		aFiles := Directory.GetFiles(cBaseDir , "*." + cName + ".vnfs")
		IF aFiles:Length == 1
			RETURN aFiles[1]
		END IF
//		aFiles := Directory.GetDirectories(cBaseDir)
