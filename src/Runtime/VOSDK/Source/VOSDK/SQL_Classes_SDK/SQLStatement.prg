...
#ENDIF
            ...
            lRet := TRUE
        ELSE
            SELF:MakeErrorInfo(SELF, __FUNCTION__, nRetCode)
        ENDIF


    ELSE
        // Statement already allocated
        SELF:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__STMT_ALLOC ), __FUNCTION__ )
    ENDIF
...
#ENDIF
    ...
    IF lRet
        oErrInfo:ErrorFlag := FALSE
    ELSE
        SELF:MakeErrorInfo(SELF,   __FUNCTION__,0)
    ENDIF
...
#ENDIF
    ...
    IF hStmt != SQL_NULL_HSTMT
        ...
        ENDIF
    ELSE
        SELF:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__NO_STMT ), __FUNCTION__ )
    ENDIF
    RETURN xRet
...
ASSIGN StatementHandle( uVal )
    ...
    IF hStmt = SQL_NULL_HSTMT
        ...
        RETURN SELF:hStmt
    ELSE
        SELF:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__STMT_NOT_ALLOC ), __FUNCTION__ )
    ENDIF
    RETURN
