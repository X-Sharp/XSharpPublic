...
// Devexpress Controls have paint problems, when old win32 styles are modified
    ...
    STATIC METHOD GetWindowStyle(hwnd AS IntPtr) AS LONG
	...
	[DllImport("User32.dll")];
	STATIC METHOD SendMessage(hwnd AS IntPtr, Msg AS DWORD, wParam AS DWORD, lParam AS LONG) AS LONG 

	[DllImport("User32.dll")];
	STATIC METHOD SendMessage(hwnd AS IntPtr, Msg AS DWORD, wParam AS DWORD, lParam AS IntPtr) AS LONG 
