...
BEGIN NAMESPACE MacroCompilerTest
	...
	FUNCTION TestMacro() AS VOID
	    SetMacroCompiler(typeof(IMacroCompiler))
    ...
    FUNCTION EvalMacro(mc AS IMacroCompiler, src AS STRING, args PARAMS OBJECT[]) AS USUAL
    ...
    FUNCTION RunTests(mc AS IMacroCompiler) AS VOID
...
//        TestMacro(mc, e"{|a,b| asdgfafd(123) }", Args(), null, null, ErrorCode.NotAMethod)
    ...
    FUNCTION RunPerf(mc AS IMacroCompiler, src AS STRING) AS VOID
    ...
    FUNCTION TestMacro(mc AS IMacroCompiler, src AS STRING, args AS OBJECT[], expect AS USUAL, t AS Type) AS LOGIC
        TRY
            TotalTests += 1
            ...
            if cb == NULL
                TotalFails += 1
                Console.WriteLine("[FAIL] Could not compile {0}", src)
                RETURN FALSE
            ENDIF
            VAR res := cb:EvalBlock(args)
            ...
            ELSE
                TRY
                    LOCAL r := res AS USUAL
                    LOCAL e := expect AS USUAL
                    match := r = e .OR. res = expect
                CATCH
                    match := res = expect
                END
            END
           IF (match) //.AND. ((t == NULL) || (t == res?:GetType()) || (t == typeof(OBJECT) .AND. res == NULL .AND. expect == NULL))
                TotalSuccess += 1
...
/*        catch e as Exception
    ...
    FUNCTION CreateMacroCompiler() AS IMacroCompiler
