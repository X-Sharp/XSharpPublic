...
begin namespace Vulcan
	...
	structure __Usual implements System.IConvertible,System.IComparable
		...
		private  constructor(a as __Array)
			self:flags		:= UsualFlags{UsualDataType.__Array}
			self:valueData	:= UsualData{}
			self:refData	:= a
		return

		private  constructor(dt as System.DateTime)
			self:flags		:= UsualFlags{ UsualDataType.__VODate}
			self:valueData	:= UsualData{}
			self:refData	:= dt
		return
				...
				if (o:GetType() == typeof(__Usual))
					...
					case System.TypeCode.UInt32
						if ((DWord)o  <= 0x7fffffff)
							...
							self:_usualType := UsualDataType.__VOFloat
					...
					case System.TypeCode.DateTime 
						self:_usualType := UsualDataType.__VODate
					...
					if ((typeCode == System.TypeCode.Object) .and. (vartype== typeof(__Array)))
						self:_usualType := UsualDataType.__Array
						self:refData  := o
					endif
		...
		internal property Is__Array   as Logic GET self:usualType == UsualDataType.__Array
		internal property IsByRef   as Logic GET self:flags:IsByRef
		internal property IsDate	as Logic GET self:usualType == UsualDataType.__VODate
		...
		internal property Is__VOFloat   as Logic GET self:usualType == UsualDataType.__VOFloat
		internal property IsValueType as LOGIC GET ! SELF:IsReferenceType

		internal property IsReferenceType as LOGIC
			GET
				if self:_usualType == UsualDataType.__Array
					return true
