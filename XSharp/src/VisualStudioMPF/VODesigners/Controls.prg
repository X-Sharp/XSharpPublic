...
INTERNAL CLASS DesignWindow INHERIT Panel
	...
	PROTECTED METHOD WndProc(m REF Message) AS VOID
		DO CASE
		CASE WindowDesignerBase.HandleWndProc(SELF:oItem , @m)
			RETURN
...
INTERNAL CLASS DesignPushButton INHERIT Button
	...
	RETURN
	PROTECTED METHOD WndProc(m REF Message) AS VOID
		IF SELF:lTestMode .or. .not. WindowDesignerBase.HandleWndProc(SELF:oItem , @m)
			SUPER:WndProc(@m)
		END IF		
	RETURN
...
INTERNAL CLASS DesignDataColumn INHERIT PictureBox
