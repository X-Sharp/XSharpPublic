...
//		oItem:nMenuID := Funcs.Val(cValue)
		...
		IF nAt != 0
			oItem:aProperties:Add("EventName" , cValue:Substring(0, nAt - 1):Trim())
			oItem:aProperties:Add("ID" , cValue:SubString( nAt + 2):Trim())
		ELSE
	...
	PROTECTED STATIC METHOD __ReadNextVNMnuAccel(aBytes AS BYTE[] , nPos REF INT) AS MenuAccelerator
		...
		IF bModif >= 48 .and. bModif <= 55
			...
			DO WHILE aBytes[nPos] != 13
				cAccelerator +=  ((char) aBytes[nPos]):ToString()
				nPos ++
			END DO
			nPos ++
...
//			IF .not. oPair:Name:Name:EndsWith("ID")
	...
	STATIC CONSTRUCTOR()
		...
		FOR n := 0x41 UPTO 0x5A	// A..Z
			oCollection:Add( ((Char)n):ToString() , n)
		NEXT
