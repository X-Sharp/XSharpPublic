...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class OverloadResolution
    {
             ...
             where TMember : Symbol
        {
            ...
            if (allowUnexpandedForm && normalResult.Result.IsValid && IsValidParams(_binder, leastOverriddenMember, false, out elementType) && Compilation.Options.HasRuntime)
            {
        ...
        {
            TypeWithAnnotations elementType;
            if (result.IsValid && IsValidParams(_binder, constructor, false, out elementType) && Compilation.Options.HasRuntime)
            {
                // Find Params argument
                BoundExpression paramsArg = null;
        ...
        private BetterResult VoBetterOperator(BinaryOperatorSignature op1, BinaryOperatorSignature op2, BoundExpression left, BoundExpression right, ref CompoundUseSiteInfo<AssemblySymbol> useSiteInfo)
        {
            ...
            if ((op1.Kind & BinaryOperatorKind.OpMask) == (op2.Kind & BinaryOperatorKind.OpMask))
            {
                ...
                if (!object.ReferenceEquals(left.Type, null) && !object.ReferenceEquals(right.Type, null))
                {
                    ...
                    if (Compilation.Options.HasOption(CompilerOption.VOSignedUnsignedConversion, left.Syntax) || // vo4
