...
BEGIN NAMESPACE XSharpModel
    ...
    PARTIAL CLASS ParserV2
        ...
        PROTECTED _EntityList    AS IList<XElement>
        PROTECTED _EntityStack   AS Stack<XElement>
        PROTECTED _BlockList     AS IList<XBlock>
        PROTECTED _BlockStack    AS Stack<XBlock>
        PROTECTED _PPBlockStack  AS Stack<XBlock>
        PROTECTED _lastToken    AS IToken
        PROTECTED _locals       AS LOGIC
        PROTECTED _blocks       AS LOGIC
        
        PROPERTY CurrentEntity      AS Xelement GET IIF(_EntityStack:Count > 0, _EntityStack:Peek(), NULL_OBJECT)
        PROPERTY CurrentBlock       AS XBlock   GET IIF(_BlockStack:Count > 0, _BlockStack:Peek(), NULL_OBJECT)
        PROPERTY CurrentEntityKind  AS Kind     GET IIF(_EntityStack:Count > 0, CurrentEntity:Kind, Kind:Unknown)
        ...
        METHOD Parse( tokenStream AS ITokenStream, oFile AS XFile, lBlocks AS LOGIC, lLocals AS LOGIC) AS VOID
            ...
            _EntityStack:Push(XType.CreateGlobalType(NULL))
            _index := 0
            DO WHILE ! SELF:Eoi()
        ...
        METHOD ParseBlock() AS LOGIC
            ...
            IF nMiddle > 0
                // Do something
                IF SELF:_blocks .AND. _BlockStack:Count > 0
