...
BEGIN NAMESPACE XSharp.VFP.UI
	...
	STATIC CLASS __VFPCreateObject
		...
		/// </summary>
		STATIC METHOD Create( prefix AS STRING, className ) AS OBJECT CLIPPER
			LOCAL allArgs := _ARGS() AS USUAL[]
			// allArgs[1] = prefix, allArgs[2] = className, allArgs[3..] = constructor params
			LOCAL callArgs := List<Usual>{}
			callArgs:Add( prefix + (STRING) allArgs[2] )
