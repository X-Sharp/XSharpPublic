...
namespace XSharp.Project
{
    internal class XSharpSdkProjectNode : XSharpProjectNode
    {
        ...
        private bool suspendDuplicateCheck = false;

        protected override void ProcessFiles()
        {
            // This adds all the file nodes of the project. Checking every single node for a duplicate
            // build item is far too expensive, so we do that once, when they are all in place.
            suspendDuplicateCheck = true;
            try
            {
                base.ProcessFiles();
            }
            ...
            {
                suspendDuplicateCheck = false;
            }
            RemoveDuplicateBuildItems();
        }

        public override void OnAfterProjectOpen(object sender, AfterProjectFileOpenedEventArgs e)
        {
            var wasLoading = this.IsLoading;
            base.OnAfterProjectOpen(sender, e);         // this clears IsLoading
            if (wasLoading)
            {
                // catch the nodes that were added while loading, after ProcessFiles() had finished
                RemoveDuplicateBuildItems();
            }
        }

        /// <summary>
        /// Remove the items from the project file that are also generated by the wildcards in the SDK targets.
        /// </summary>
        /// <remarks>
        /// This check used to run inside OnItemAdded(), once per file node. That is very expensive for SDK
        /// projects: RemoveItem() marks the BuildProject dirty, so the ReevaluateIfNecessary() for the next
        /// file node triggers a complete MSBuild re-evaluation, which re-expands all the recursive wildcards
        /// from XSharp.SDK.Props. For a project that was converted from the legacy format nearly every file
        /// is a duplicate, so that happened for nearly every file in the project.
        /// </remarks>
        private void RemoveDuplicateBuildItems()
        {
            ...
            var candidates = new List<MSBuild.ProjectItem>();
            foreach (var item in project.Items)
            {
                if (item.IsImported)
                    imported.Add(item.ItemType + ":" + item.EvaluatedInclude);
                else
                    candidates.Add(item);
            }
            var removed = 0;
            ...
            {
                if (!imported.Contains(item.ItemType + ":" + item.EvaluatedInclude))
                    continue;
                try
                {
                ...
                {
                    Logger.Exception(e, $"Could not remove duplicate item {item.ItemType} {item.EvaluatedInclude}");
                }
            }
            if (removed > 0)
            {
                Logger.Information($"Removed {removed} duplicate item(s) from project {this.Caption}");
                project.ReevaluateIfNecessary();
            }
        }
        ...
        public override void OnItemAdded(HierarchyNode parent, HierarchyNode child)
        {
            ...
            if (suspendDuplicateCheck || this.IsLoading)
            {
                // duplicates are removed in one pass, see RemoveDuplicateBuildItems()
                return;
            }
