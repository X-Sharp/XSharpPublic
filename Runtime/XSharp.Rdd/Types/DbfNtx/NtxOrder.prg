    ...
    INTERNAL CLASS NtxOrder INHERIT BaseIndex IMPLEMENTS IRddSortWriter
        ...
        PUBLIC PROPERTY Expression AS STRING GET _KeyExpr
                ...
                PUBLIC PROPERTY OrderName AS STRING GET _orderName
                    ...
                    INTERNAL PROPERTY Recno AS DWORD GET (DWORD)_oRdd:Recno
                        ...
                        PUBLIC CONSTRUCTOR(oRDD AS DBFNTX )
                            ...
                            SELF:_hFile := NULL
                            SELF:_oRdd := oRDD
                            SELF:_Header := NtxHeader{ oRDD:_hFile }
            ...
            PRIVATE METHOD _goTo(recno AS DWORD ) AS LOGIC
                ...
                result := TRUE
                SELF:_keySave(recno)
                IF ((SELF:_goRecord(SELF:_knownKeyBuffer, SELF:_keySize, recno) != recno) .AND. (SELF:_goRecord(NULL, 0, recno) != recno))
                    IF (((!SELF:_Unique) .AND. (!SELF:_Conditional)) .AND. (!SELF:_Partial))
            ...
            PRIVATE METHOD _locateKey(abKey AS BYTE[] , uiLen AS LONG , uiMode AS NtxSearchMode ) AS DWORD
                SELF:_TopStack := 0u
                ...
                ELSE
                    ...
                    ENDIF
                ENDIF
                RETURN SELF:_locate(abKey, uiLen, uiMode, (INT)SELF:_firstPageOffset)
                
                
            PRIVATE METHOD _locate(abKey AS BYTE[] , uiLen AS LONG , uiMode AS NtxSearchMode , lPage AS LONG ) AS DWORD
                LOCAL count AS LONG
                LOCAL ntxPage AS NtxPage
                LOCAL nodeCount AS LONG
                LOCAL node AS NtxItem
                ...
                CASE NtxSearchMode.Right
                    IF (SELF:_Descending)
                        ...
                        WHILE loCount < hiCount
                            ...
                            IF (SELF:_oRdd:StringCompare(node:KeyBytes, abKey, uiLen) >= 0)
                                loCount := count + 1
                            ELSE
                                hiCount := count
                            ENDIF
                ...
                CASE NtxSearchMode.LeftFound
                    ...
                    count := loCount
                    node:Fill(count, ntxPage)
                    IF ((uiMode == NtxSearchMode.Left) .AND. (SELF:_oRdd:StringCompare(node:KeyBytes, abKey, uiLen) == 0))
                        uiMode := NtxSearchMode.LeftFound
                    ENDIF
            ...
            node:Fill(count, ntxPage)
            IF (node:PageNo != 0)
                RETURN SELF:_locate(abKey, uiLen, uiMode, node:PageNo)
            ENDIF
            IF (count < nodeCount)
                BEGIN SWITCH uiMode
            CASE NtxSearchMode.LeftFound
                ...
                END SWITCH
        ELSE
            IF (uiMode == NtxSearchMode.LeftFound)
                WHILE (SELF:_TopStack != 0) .AND. (SELF:_ntxStack[SELF:_TopStack]:Pos == SELF:_ntxStack[SELF:_TopStack]:Count)
                    SELF:PopPage()
                END WHILE
                IF (SELF:_TopStack != 0)
...
BEGIN NAMESPACE XSharp.RDD 
	...
	INTERNAL CLASS NtxOrder INHERIT BaseIndex IMPLEMENTS IRddSortWriter
		...
		PUBLIC PROPERTY Expression AS STRING GET _KeyExpr
			...
			PUBLIC PROPERTY Condition AS STRING GET _ForExpr
				...
				PRIVATE METHOD _CondCreate(ordCondInfo AS DBORDERCONDINFO ) AS LOGIC
					...
					IF (ordCondInfo:Scoped)
						...
						ENDIF
						IF (SELF:_oRdd:_ntxList:Focus != 0)
							ntxIndex := SELF:_oRdd:_ntxList:CurrentOrder
						ENDIF
						IF (ordCondInfo:All)
							record := 1
							IF (ntxIndex != NULL)
								record := ntxIndex:_locateKey(NULL, 0, NtxSearchMode.Top)
							ENDIF
						ENDIF
					ENDIF
					IF (ordCondInfo:RecNo > 0)
					...
					IF (ordCondInfo:EvalBlock != NULL)
						hasEvalBlock := TRUE
					ENDIF
					IF ((ntxIndex != NULL) .AND. (ntxIndex:_TopStack != 0))
						result := ntxIndex:_goTo((DWORD)SELF:_oRdd:RecNo)
						IF (!result)
							RETURN result
						ENDIF
					ENDIF
			...
			PUBLIC METHOD Skip(lCount AS LONG ) AS LOGIC
				LOCAL recno AS DWORD
				LOCAL isBof AS LOGIC
				LOCAL isEof AS LOGIC
				LOCAL changedBof AS LOGIC
