...
/// <summary>VFP <c>CREATE()</c> alias — creates an instance of <paramref name="symClassName"/> via <c>CreateObject</c> with optional constructor parameters.</summary>
FUNCTION Create(symClassName,InitParamsList) AS OBJECT CLIPPER
	RETURN CreateObject(symClassName,InitParamsList)


/// <summary>Brings a top-level window to the foreground by title. Uses <c>FindWindow</c> + <c>SetForegroundWindow</c>; silently ignored if no window matches.</summary>
...
/// <summary>Hides a top-level window by title using <c>SW_HIDE</c>. Silently ignored if no window matches.</summary>
...
/// <summary>Returns <c>.T.</c> if a top-level window with the given title exists and is currently visible. VFP <c>WVISIBLE()</c> equivalent.</summary>
...
/// <summary>Returns <c>.T.</c> if a top-level window with the given title exists (visible or not). VFP <c>WEXIST()</c> equivalent.</summary>
