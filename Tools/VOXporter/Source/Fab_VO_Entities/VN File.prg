...
//		    dCreate := SToD( dt:Year:ToString() + dt:Month:ToString() + dt:Day:ToString() )
    ...
    PROTECT METHOD Scan2( ) AS VOID
        LOCAL nCpt          AS  LONG
        LOCAL dwMax         AS  LONG
        LOCAL cLine         AS  STRING
        LOCAL cTempLine     AS  STRING
        LOCAL lContinue     AS  LOGIC
        LOCAL lInComment    AS  LOGIC
        LOCAL aElements     AS  STRING[]
        LOCAL aDelim        AS  STRING[]
        LOCAL aEntKeyW      AS  STRING[]
        LOCAL cElem         AS  STRING
        LOCAL nElem         AS  LONG
        LOCAL nPos          AS  LONG
        LOCAL nStart        AS  LONG
        LOCAL nEnd          AS  LONG
        LOCAL cCurrentClass     AS  STRING
        LOCAL cCurrentNSpace    AS  STRING
	    ...
	    FOR nCpt := 0 TO dwMax-1
		    ...
		    // Was it a continued line?
		    IF lContinue
		        cLine := cTempLine + " " + cLine
		    ENDIF
		    // Is it a multiline ?
		    IF ( Right( cLine, 1 ) == ";" )
			    cTempLine := SubStr( cLine, 1, SLen( cLine ) - 1 )
			    lContinue := TRUE
			    LOOP
		    ENDIF
		    ...
		    aDelim := <STRING>{ " ", CRLF }
	        aElements := cLine:Split( aDelim, StringSplitOptions.RemoveEmptyEntries )
	        // Do we have an entity start here ?
	        FOR nElem := 0 TO aElements:Length-1
	            // Let's check a token
	            cElem := (STRING) aElements:GetValue( nElem )
	            cElem := cElem:ToUpper()
	            //cElem := aElements[ nElem ]
                //
                nPos := System.Array.IndexOf( aEntKeyW, cElem )
                //
                //IF ( nPos > -1 )
                    //IF ( cElem == "BEGIN" )
                    //ENDIF
		    ...
		    IF ( nPos > -1 )
		        ...
		        ELSE
			        ...
			        nEnd := nCpt - 1
			        //
			        AAdd( SELF:aEnt, FabVNEntity{ "",  cProto, SELF, nStart, nEnd, cCurrentClass, cCurrentNSpace, ;
	    ...
	    FOR nCpt := 0 TO dwMax-1
		    ...
		    IF ( nKeySize >= 4 ) .AND. ( AScan( aKeyw, cKeyword, nKeySize ) != 0 )
			    ...
			    ELSE
				    ...
				    AAdd( SELF:aEnt, FabVNEntity{ "",  cProto, SELF, nStart, nEnd, "", "",  ;
									    SELF:dCDate, SELF:cCTime, SELF:dBDate, SELF:cBTime } )
				    // And set the new Start
				    nStart := nCpt
			    ENDIF
    ...
    PROTECT METHOD	SortByName( ) AS VOID  
        ...
        LOCAL nCpt  AS LONG
        LOCAL nMax  AS LONG
        LOCAL oEnt1	AS	FabVNEntity
        LOCAL oEnt2	AS	FabVNEntity
        ...
        REPEAT
            ...
            FOR nCpt := 1 TO (nMax-1)
                //
                oEnt1 := (FabVNEntity)SELF:aEnt[ (DWORD)nCpt ]
                oEnt2 := (FabVNEntity)SELF:aEnt[ (DWORD)nCpt + 1 ]
