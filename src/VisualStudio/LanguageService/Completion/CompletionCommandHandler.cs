...
{
    internal partial class XSharpCompletionCommandHandler : AbstractCommandHandler
    {
        ...
        internal XSharpCompletionCommandHandler(IVsTextView textViewAdapter, ITextView textView,
            ICompletionBroker completionBroker, IBufferTagAggregatorFactoryService aggregator) : base(textViewAdapter)
        {
        ...
        public override int QueryStatus(ref Guid pguidCmdGroup, uint cCmds, OLECMD[] commands, IntPtr pCmdText)
        {
            ...
            return base.QueryStatus(ref pguidCmdGroup, cCmds, commands, pCmdText);
        }
        ...
        public override int Exec(ref Guid cmdGroup, uint nCmdID, uint nCmdExecOpt, IntPtr pvaIn, IntPtr pvaOut)
        {
            ...
            if (!handled)
            {
                if (CompletionActive)
                {
                    if (_doc.CompletionSession.SelectedCompletionSet != null)
                    {
                        completionWasSelected = _doc.CompletionSession.SelectedCompletionSet.SelectionStatus.IsSelected;
                    }
                }
        ...
        private void FilterCompletionSession(char ch)
        {
            ...
            if (_doc.CompletionSession.SelectedCompletionSet != null)
            {
                ...
                _doc.CompletionSession.SelectedCompletionSet.Filter();
                if (_doc.CompletionSession.SelectedCompletionSet.Completions.Count == 0)
                {
                    CancelCompletionSession();
                }
                else
                {
                    var props = _doc.CompletionSession.GetCompletionProperties();
                    if (ch != '\0')
                        props.Filter += ch;
                    WriteOutputMessage(" --> Selecting ");
                    _doc.CompletionSession.SelectedCompletionSet.SelectBestMatch();
                    _doc.CompletionSession.SelectedCompletionSet.Recalculate();
                }
            }
        ...
        bool CancelCompletionSession()
        {
            ...
            _doc.CompletionSession.Dismiss();
            return true;
        }
        bool CompleteCompletionSession(char ch)
        {
            if (!CompletionActive || _doc.CompletionSession.SelectedCompletionSet == null)
            {
                return false;
            }
            var session = _doc.CompletionSession;
            ...
            else
            {
                ...
                if (XEditorSettings.CompletionAutoPairs)
                {
                    caret = _doc.CompletionSession.TextView.Caret;
                    addClose = true;
                }
            }
            if (addClose)
        ...
        bool StartCompletionSession(uint nCmdId, char typedChar, bool includeKeywords = false, bool autoType = false)
        {
            ...
            if (CompletionActive)
            {
                if (!_doc.CompletionSession.IsDismissed)
                    return false;
            }
            if (!CompletionAllowed(typedChar, !autoType))
                return false;

            CompletionHelpers.GetSnippets();
            ...
            _doc.CompletionSession.Dismissed += OnCompletionSessionDismiss;
            //_completionSession.Committed += OnCompletionSessionCommitted;
            _doc.CompletionSession.SelectedCompletionSetChanged += SelectedCompletionSetChanged;
            var props = _doc.CompletionSession.GetCompletionProperties();
