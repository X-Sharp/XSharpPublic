...
//			File.Delete(cFileName)
    ...
    */
	...
	VIRTUAL METHOD DoBasicAction(oAction AS DesignerBasicAction , eAction AS DesignerBasicActionType , uData AS ActionData) AS OBJECT
		LOCAL aProperties := NULL AS NameValueCollection
		LOCAL oUndo,oRedo := NULL AS DesignerBasicAction
		LOCAL oDesign := NULL AS DBEDesignDBServer
		LOCAL oParent := NULL AS DBEDesignDBServer
		LOCAL oItem := NULL AS DBEDesignListViewItem
		LOCAL oProp := NULL AS VODesignProperty
		LOCAL oListView := NULL AS DBEListView
		LOCAL cGuid AS STRING
		LOCAL oEnumerator := NULL AS IEnumerator
...
/*			CASE oProp:cEnumType == "YESNO"
		...
		SplitFilename(SELF:oMainDesign:GetProperty("filename"):TextValue:Trim() , OUT dbfpath , OUT dbfname)
	...
	INTERNAL METHOD TranslateMacro(eType AS DBServerItemType , oIndex AS DBEDesignDBServer) AS STRING
		...
		FOR m := 0 UPTO aDesign:Count - 1
			...
			CASE eType == DBServerItemType.Index
				...
				SplitFilename(oDesign:GetProperty("filename"):TextValue:Trim() , OUT dbfpath , OUT dbfname)
				IF dbfpath == ""
					LOCAL temp AS STRING
					SplitFilename(SELF:oMainDesign:GetProperty("filename"):TextValue:Trim() , OUT dbfpath , OUT temp)
				END IF
...
/*		cLine := cLine:Replace("\r\n" , ChrW(0))
	...
	INTERNAL STATIC METHOD SplitFilename(cFullPath AS STRING , cFolder OUT STRING , cFileName OUT STRING) AS VOID
		LOCAL nAt AS INT
	...
	METHOD Open(cFileName AS STRING) AS LOGIC
		...
		IF SELF:oProject == NULL
			Funcs.WarningBox("Cannot find the project, the editor will not be able to locate the fieldspecs")
		END IF
...
/*		aFieldSpecs := VOFieldSpecEditor.ReadFieldSpecsInModule(cModule)
		...
		IF aFieldSpecs:Count != 0 .and. lSaveFieldSpecs
			VOFieldSpecEditor.SaveToXml(cModule + ".FieldSpecs.vnfs" , aFieldSpecs)
            IF SELF:oProject != NULL_OBJECT
			    SELF:oProject:AddItem(cModule + ".FieldSpecs.vnfs")
	...
	METHOD ReadAllAvailableFieldspecs(cModule AS STRING) AS VOID
		...
		LOCAL oBinary AS XSharp.ProjectAPI.VOBinary
        ...
        IF SELF:oProject == NULL_OBJECT
            RETURN
        ENDIF
		aVnfs := SELF:oProject:GetBinaries(XSharp.ProjectAPI.VOBinaryType.FieldSpec , TRUE)
		
/*		LOCAL aFiles AS STRING[]
//		aFiles := FileInfo{cModule}:Directory:GetFiles("*.vnfs")
		aFiles := Directory.GetFiles(FileInfo{cModule}:DirectoryName , "*.vnfs")
		...
		FOR n := 0 UPTO aVnfs:Count - 1
			oBinary := aVnfs[n]
			cFileName := oBinary:Name
			lModule := Funcs.GetModuleFilenameFromBinary(cFileName):ToUpper() == cModule:ToUpper()
			IF cFileName:ToUpper():Contains(".FIELDSPECS.VNFS")
				aDesign := VOFieldSpecEditor.OpenXml(cFileName , NULL)
				FOR m := 0 UPTO aDesign:Count - 1
					oDesign := (FSEDesignFieldSpec)aDesign[m]
					SELF:aAvailableFieldSpecs:Add(oDesign)
					IF lModule
						SELF:aFieldSpecsInModule:Add(oDesign)
					END IF
				NEXT
			ELSE
				oDesign := VOFieldSpecEditor.OpenVNfs(cFileName , (VOFieldSpecEditor)NULL)
				IF oDesign != NULL
...
CLASS DBServerCode
	...
	METHOD ReadDed(cDirectory AS STRING) AS LOGIC
		...
		LOCAL lInMacros := FALSE AS LOGIC
...
//					IF cLine:ToUpper():StartsWith("METHOD") .and. cLine:ToUpper():Contains("INIT")
	...
	METHOD TranslateTokens(cMacro REF STRING) AS VOID
		...
		cMacro := cMacro:Replace("\r" , e"\0")
		cMacro := cMacro:Replace("\n" , e"\0")
		cMacro := cMacro:Replace("\t" , e"\t")
