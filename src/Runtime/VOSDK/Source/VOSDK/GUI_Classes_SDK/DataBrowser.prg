...
CLASS DataBrowser INHERIT Control
    ...
    METHOD __AutoLayout() AS VOID STRICT
        ...
        IF lLinked
            ...
            IF oParent IS __FormFrame var oFrame
                oWindow := oFrame:Owner
            ENDIF

            IF oWindow IS DataWindow VAR oDW //vor die Loop gesetzt, da sich das Ergebnis in der Loop nicht �ndert
                oDatawin := oDW
            ELSE
    ...
    METHOD __BeginEditField(hWin AS PTR, dwChar AS DWORD) AS VOID STRICT
        ...
        IF (strucEditRecord != NULL_PTR) .AND. (strucEditField != NULL_PTR)
            ...
            ELSE
                ...
                IF oEdit IS Control VAR oEditControl
                    oCellEdit := oEditControl
    ...
    METHOD __FindColumn(nIndex AS USUAL) AS DWORD STRICT
        ...
        CASE dwType == SYMBOL .OR. dwType == STRING
            ...
            RETURN 0
            //SE-060526
        CASE nIndex IS DataColumn VAR oDC
            oDCol := oDC
            dwCount := ALen(aColumn)
            FOR dwI := 1 UPTO dwCount
            ...
            IF uMsg == WM_COMMAND
                ...
                CASE CN_ROFLDDBLCLK
                    IF IsMethod(SELF, #CellDoubleClick)
                        Send(SELF, #CellDoubleClick, oEvent)
    ...
    CONSTRUCTOR(oOwner, xID, oPoint, oDimension)
        ...
        IF oWin IS DataWindow VAR oDW2
            oDataWin := oDW2
    ...
    METHOD Refresh()
        ...
        /// Use IsInstanceOf because RddClasses is not referenced
        IF IsInstanceOf(oDataServer, #DBServer)
...
CLASS DataColumn INHERIT VObject
    ...
    METHOD ChangeTextColor(oColor, kWhere)
        ...
        ELSEIF oColor IS Color var oCol
            oNewColor := oCol
        ELSE
    ...
    ASSIGN HyperLabel(oNewHL)
        ...
        ELSEIF oNewHL IS STRING var strHL
            oHyperLabel := HyperLabel{String2Symbol(strHL)}
            lExplicitHL := TRUE
            SELF:Caption := strHL
        ELSEIF oNewHL IS Symbol VAR symHL
            oHyperLabel := HyperLabel{symHL}
            lExplicitHL := TRUE
            SELF:Caption := symHL
        ELSEIF IsNil(oNewHL)
    ...
    CONSTRUCTOR(nWidth, xColumnID)
        ...
        IF IsNil(nWidth)
            iSize := 16
        ELSEIF nWidth IS FieldSpec var oFS
            iSize := __GetFSDefaultLength(oFS)
            SELF:FieldSpec := oFS
        ...
        IF xColumnID IS HyperLabel var oHL
            SELF:HyperLabel := oHL
        ELSEIF xColumnID IS STRING var strHL
            SELF:HyperLabel := HyperLabel{String2Symbol(strHL),strHL}
        ELSEIF xColumnID IS SYMBOL var symHL
            SELF:HyperLabel := HyperLabel{symHL,Symbol2String(symHL)}
        ENDIF
...
FUNCTION __WCGBChildProc(hWnd AS PTR, uMsg AS DWORD, wParam AS DWORD, lParam AS LONGINT) AS LONGINT /* WINCALL */
    ...
    IF (uMsg == WM_CHAR)
        ...
        IF oBrowser IS DataBrowser VAR oBrow
            oBrow:__LastChar := wParam
        ENDIF
...
FUNCTION __CellEditProc(hWnd AS PTR, uMsg AS DWORD, wParam AS DWORD, lParam AS LONGINT) AS /* LOGIC */ INT //_WINCALL
    ...
    IF (oControl != NULL_OBJECT)
        ...
        IF oOwner IS DataBrowser var oDB
            lpfnDefaultProc := oDB:ptrControlDefaultProc
            IF oDB:__EditDispatch(uMsg, wParam, lParam)
                RETURN 1 // TRUE
            ENDIF
