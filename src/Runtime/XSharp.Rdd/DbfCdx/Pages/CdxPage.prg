...
INTERNAL ABSTRACT CLASS CdxPage
    ...
    INTERNAL PROPERTY PageNo AS DWORD  GET _nPage SET _nPage := VALUE
    INTERNAL PROPERTY Bag    AS CdxOrderBag GET _bag
        // For debugging
    INTERNAL PROPERTY PageNoX AS STRING GET _nPage:ToString("X8")

    INTERNAL STATIC METHOD PageIsValid(pageNo as DWORD) AS LOGIC
        RETURN pageNo != MISSING_PAGE .and. pageNo != 0
    ...
    PROTECTED INTERNAL CONSTRUCTOR( bag AS CdxOrderBag )
        ...
        ELSE
            NOP
        ENDIF
    PROTECTED INTERNAL CONSTRUCTOR( bag AS CdxOrderBag , nPage AS DWORD , buffer AS BYTE[])
        SELF(bag)
        SELF:_nPage  := nPage
        SELF:IsHot  := FALSE
