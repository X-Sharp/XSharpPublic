...
PARTIAL CLASS VOFieldSpecEditor INHERIT DesignerBase
	...
	PROTECT oProject AS Vulcan.CodeModel.Project
...
//		LOCAL oDesign AS FSEDesignFieldSpec
		...
		IF SELF:GetSaveFileStreams(cFileName , cPrgFileName , oPrgStream , lVnfrmOnly)
			...
			oModule := Vulcan.CodeModel.Projects.Find(cPrgFileName)
...
*/
	...
	METHOD Open(cFileName AS STRING) AS LOGIC
		LOCAL aVnfs AS IList<Vulcan.CodeModel.VOBinary>
		LOCAL oDesign AS FSEDesignFieldSpec
		LOCAL aFieldSpecs AS ArrayList
		LOCAL lModule AS LOGIC
		LOCAL lMerge AS LOGIC
		LOCAL n AS INT
		...
		TRY
			SELF:oProject := Vulcan.CodeModel.Projects.Find(Funcs.GetModuleFilenameFromBinary(cFileName) + ".prg"):Parent
		END TRY
...
//			MessageBox.Show("Fieldspec definitions read from xml file: " + cFileName , "FieldSpec Editor")
		...
		aVnfs := SELF:oProject:GetBinaries(Vulcan.CodeModel.VOBinaryType.FieldSpec , FALSE)
