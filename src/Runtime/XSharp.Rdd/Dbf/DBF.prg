...
#region STATIC properties and fields
    ...
    OVERRIDE METHOD Create(info AS DbOpenInfo) AS LOGIC
        ...
        IF SELF:_Fields:Length == 0
            RETURN FALSE
        ENDIF
        SELF:SetOpenInfo(info, _Extension, TRUE)
        //
        SELF:_Hot := FALSE
        //
#ifdef INPUTBUFFER
    ...
    OVERRIDE METHOD Open(info AS XSharp.RDD.Support.DbOpenInfo) AS LOGIC
        ...
        //
        isOK := FALSE
        SELF:SetOpenInfo(info, _Extension, FALSE)
        SELF:_Hot := FALSE
#ifdef INPUTBUFFER
        SELF:_hFile    := FOpen(SELF:_FileName, SELF:OpenInfo:FileMode | FO_UNBUFFERED)
#else
