...
#ENDIF
    ...
    IF ! ((SqlColumnAttributes)SELF:ColumnAttributes( nIndex )):Updatable
        oStmt:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__UPDATE_COL ), __FUNCTION__ )
        SELF:Error( oStmt:ErrInfo )
        RETURN NIL
    ENDIF
        ...
        IF !lRet
            IF oDF:__FieldSpec:Status != NULL_OBJECT
                ...
                oStmt:__GenerateSQLError( oDF:__FieldSpec:__HyperLabel:Description ,  ;
                    __FUNCTION__)
            ELSE
                oStmt:__GenerateSQLError( __CavoStr( __CAVOSTR_SQLCLASS__BADVALID ), __FUNCTION__ )
            ENDIF
        ENDIF
    ENDIF
    RETURN lRet
